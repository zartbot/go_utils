...
// ReadUint32 : LittleEndian uint32 function
func (r *Reader) ReadUint32LE() (uint32, error) {
	const Length = 4
	if r.Pos+Length > r.Length {
		return 0, errors.New("read UINT32 data out-of-boundary")
	}
	d := binary.LittleEndian.Uint32(r.data[0:Length])
	r.movePos(Length)
...
// ReadUint64 : LittleEndian uint64 function
func (r *Reader) ReadUint64LE() (uint64, error) {
	const Length = 8
	if r.Pos+Length > r.Length {
		return 0, errors.New("read UINT64 data out-of-boundary")
	}
	d := binary.LittleEndian.Uint64(r.data[0:Length])
	r.movePos(Length)
...
// FetchUint16 : BigEndian uint16 function, but does not move Pos
...
// FetchUint32 : BigEndian uint32 function, but does not move Pos
...
// FetchUint64 : BigEndian uint64 function, but does not move Pos
